...
import {
    T, Tank, GameMap, customMap, createBot, simulateNavigation,
    BOT_KEYS, AI_ROLES, pickRole, CONFIG, distance,
} from './helpers.js';
...
describe('AI Roles – Cavalry', () => {
    ...
    it('heads straight toward the enemy tower', () => {
        ...
        const result = simulateRole(bot, objective, map,
            { seconds: 25, objective });
        assert.ok(result.reachedTarget,
            `cavalry should reach tower, got dist=${result.finalDist.toFixed(1)}`);
    });
...
describe('AI Roles – Sniper', () => {
    ...
    it('stops at firing range from the enemy tower', () => {
        ...
        assert.ok(distToObj >= CONFIG.SNIPER_MIN_RANGE - 1,
    ...
    it('only engages enemies within close range', () => {
        ...
        const enemy = new Tank(9, '#33d', '#239');
        enemy.team = 2; enemy.alive = true;
        enemy.x = 48.5; enemy.y = 32.5;  // near tower, far from sniper
...
describe('AI Roles – Defender', () => {
    ...
    it('stays near the friendly tower when no enemies', () => {
        ...
        const objective = { x: 50.5, y: 32.5, alive: true };
        const bot = createRoleBot(14.5, 32.5, 0, map, AI_ROLES.DEFENDER,
            { friendlyTower });
        ...
        const distToFriendly = Math.hypot(friendlyTower.x - bot.tank.x,
                                           friendlyTower.y - bot.tank.y);
        assert.ok(distToFriendly < CONFIG.DEFENDER_PATROL_RADIUS + 5,
    ...
    it('intercepts enemies approaching the friendly tower', () => {
        ...
        const enemy = new Tank(9, '#33d', '#239');
        enemy.team = 2; enemy.alive = true;
        enemy.x = 22.5; enemy.y = 32.5; // within DEFENDER_ENGAGE_RANGE of tower
...
describe('AI Roles – Scout', () => {
    ...
    it('takes a wider path than cavalry (offset from direct line)', () => {
        ...
        assert.ok(scoutWider >= 5,
            `scout should take wider path than cavalry in >=5/10 trials, got ${scoutWider}`);
    });
    ...
    it('eventually reaches the enemy tower', () => {
        ...
        const result = simulateRole(bot, objective, map,
            { seconds: 45, objective });
        assert.ok(result.reachedTarget,
            `scout should reach tower, got dist=${result.finalDist.toFixed(1)}`);
    });
    ...
    it('only engages enemies within close range', () => {
        ...
        const enemy = new Tank(9, '#33d', '#239');
        enemy.team = 2; enemy.alive = true;
        enemy.x = 30.5; enemy.y = 40.5;  // > 6 tiles away
...
describe('AI Roles – team simulation with roles', () => {
    ...
    it('all roles function in a mixed team on random maps', () => {
        ...
        for (let trial = 0; trial < 5; trial++) {
            ...
            const roles = [AI_ROLES.CAVALRY, AI_ROLES.SNIPER,
                           AI_ROLES.DEFENDER, AI_ROLES.SCOUT];
            const bots = roles.map((role, i) => {
                const sp = map.getBaseSpawnPoint(tp1.x, tp1.y);
                const bot = createRoleBot(sp.x, sp.y, 0, map, role,
                    { friendlyTower });
                bot.tank.team = 1;
        ...
        assert.ok(failures <= 2,
            `at most 2 non-defender bots should fail to make progress, got ${failures}`);
    });
