...
import { customMap, GameMap, Pathfinder, T, wallH, wallU, zigzag } from "./helpers.js";
...
describe("Pathfinder", () => {
    it("finds a straight-line path on open terrain", () => {
        ...
        for (const wp of path) {
            assert.ok(map.isPassable(wp.x, wp.y), `waypoint (${wp.x},${wp.y}) should be passable`);
        }
    });
    ...
    it("prefers tiles away from walls (wall-cost weighting)", () => {
        ...
        const middleCount = path.filter((wp) => Math.floor(wp.y) >= 30 && Math.floor(wp.y) <= 32).length;
        const ratio = middleCount / path.length;
        assert.ok(ratio > 0.4, `should prefer centre, got ${(ratio * 100).toFixed(0)}% in middle rows`);
    });
    ...
    it("invalidate() rebuilds wall costs after terrain destruction", () => {
        ...
        assert.ok(path2.length <= len1, `gap path (${path2.length}) should be <= detour path (${len1})`);
    });
