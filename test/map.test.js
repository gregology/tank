...
import { CONFIG, T, GameMap, distance } from './helpers.js';
...
describe('Map generation', () => {
    ...
    it('has water around the edges', () => {
        ...
        for (const [x, y] of corners) {
            ...
            assert.ok(t === T.DEEP_WATER || t === T.SHALLOW_WATER,
                `corner (${x},${y}) should be water, got ${t}`);
        }
    });
    ...
    it('has buildings for cover', () => {
        ...
        for (let y = 0; y < 64; y++) {
            for (let x = 0; x < 64; x++) {
                const t = map.getTile(x, y);
                if (t === T.BLDG_SMALL || t === T.BLDG_MEDIUM || t === T.BLDG_LARGE)
                    buildings++;
            }
        }
...
describe('Tower positions', () => {
    ...
    it('creates sand bases around towers', () => {
        ...
        const gx = Math.floor(tp1.x), gy = Math.floor(tp1.y);
        let sandCount = 0;
    ...
    it('clears terrain around bases (radius 10)', () => {
        ...
        for (let dy = -8; dy <= 8; dy++) {
            for (let dx = -8; dx <= 8; dx++) {
                ...
                assert.ok(t !== T.HILL && t !== T.ROCK,
                    `(${gx + dx},${gy + dy}) near base should not be hill/rock`);
            }
        }
    });
    ...
    it('clears a direct path between towers', () => {
        const map = new GameMap();
        const [tp1, tp2] = map.findTowerPositions();
        const dx = tp2.x - tp1.x, dy = tp2.y - tp1.y;
