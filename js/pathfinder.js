...
class MinHeap {
    ...
    push(node)    { this.d.push(node); this._up(this.d.length - 1); }
    pop() {
        const top = this.d[0];
...
export class Pathfinder {
    ...
    findPath(sx, sy, gx, gy) {
        const w = this._w, h = this._h, map = this.map;
        ...
        const key = (x, y) => y * w + x;
        const sKey = key(s.gx, s.gy), gKey = key(g.gx, g.gy);
        if (sKey === gKey) return [];
        ...
        while (open.size > 0) {
            ...
            for (const [dx, dy, baseCost] of DIRS) {
                ...
                if (dx !== 0 && dy !== 0) {
                    if (!map.isPassable(cx + dx + 0.5, cy + 0.5) ||
                        !map.isPassable(cx + 0.5, cy + dy + 0.5)) continue;
                }
                ...
                fArr[nk] = f;
                if (!inOpen[nk]) { open.push({ k: nk, f }); inOpen[nk] = 1; }
            }
        }
        ...
        for (let r = 1; r < 6; r++) {
            for (let dy = -r; dy <= r; dy++) {
                for (let dx = -r; dx <= r; dx++) {
                    ...
                    if (this.map.isPassable(nx + 0.5, ny + 0.5))
                        return { x: nx, y: ny };
                }
            }
        }
