/**
 * Respawn system — revive dead tanks and re-roll their vehicle.
 *
...
export function handleRespawns(game, dt) {
    for (const t of game.allTanks) {
        ...
        if (t.respawnTimer <= 0) {
            ...
            if (sp) t.respawnAt(sp.x, sp.y, game.rng);
            t.alive = true;
            t.flashTimer = 1;
            // Re-randomise vehicle type on respawn.
            t.vehicleType = pickVehicleType(game.typeDef.vehicles, game.rng);
            // Reset per-life AI state (via the public world-model handle).
